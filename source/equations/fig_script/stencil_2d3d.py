...
#     ax.plot(xb, yb, color="lightgrey", linewidth=2, label=r"Mesh") 
    #points
    xn = [0,0, 0, 1, -1, -0.9, 0.9]
    yn = [0,1, -1,0,  0, -0.8, 0.8]
    ax.plot(xn, yn, "D", color="k", linewidth=1, label="Points")
    #link lines
    xn = [0, 0, np.nan, -1, 1, np.nan, -0.9, 0.9]
    yn = [-1,1, np.nan,  0, 0, np.nan, -0.8, 0.8]
    ax.plot(xn, yn, color="k", linewidth=2)
    # text
    ax.text(0.02, -0.02, r'$i,j,k$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text(1.02, -0.02, r'$i+1,j,k$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text(-0.98, -0.02, r'$i-1,j,k$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text(0.02, 0.98, r'$i,j+1,k$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text(0.02, -1.02, r'$i,j-1,k$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text(-0.88, -0.8, r'$i,j,k+1$', 
            fontsize=16, color='black', ha='left', va='top')
    ax.text( 0.9, 0.78, r'$i,j,k-1$', 
            fontsize=16, color='black', ha='left', va='top')
    # stencil value
    ax.text(-0.02, 0.02, r'-6',weight='bold', 
    ...
    ax.text(-0.02, -0.98, r'1',weight='bold',  
            fontsize=20, color='black', ha='right', va='bottom')
    ax.text(-0.88, -0.8, r'1',weight='bold',  
            fontsize=20, color='black', ha='right', va='bottom') 
    ax.text( 0.9,  0.8, r'1',weight='bold',  
            fontsize=20, color='black', ha='right', va='bottom') 
